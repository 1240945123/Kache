...
class PreferenceParserTest(unittest.TestCase):
    ...
    def test_forbidden_categories_extracts_multiple_chinese_quoted_categories(self):
        ...
        self.assertEqual(rule.params["forbidden_categories"], ["钢材", "煤炭"])
    ...
    def test_cross_day_no_drive_window(self):
        ...
        self.assertEqual(rule.strength, RuleStrength.HARD)
        self.assertEqual(rule.params["start_hour"], 23)
        self.assertEqual(rule.params["end_hour"], 6)
        self.assertTrue(rule.params["crosses_day"])
        self.assertEqual(rule.params["restrictions"], ["no_order", "no_deadhead"])

    def test_daily_rest_hours(self):
        rule = _only_rule("每天至少有一段连着停车休息满5小时")

        self.assertEqual(rule.rule_type, RuleType.DAILY_REST)
        self.assertEqual(rule.params["min_hours"], 5)
        self.assertTrue(rule.params["continuous"])
    ...
    def test_haul_and_pickup_distance_limits_in_one_text(self):
        ...
        self.assertEqual([rule.params["max_km"] for rule in rules], [20, 300])
    ...
    def test_monthly_visit_days_coordinate_target_with_radius(self):
        ...
        self.assertEqual(rule.params["min_days"], 2)
        self.assertEqual(rule.params["lat"], 31.2304)
        self.assertEqual(rule.params["lng"], 121.4737)
        self.assertEqual(rule.params["radius_km"], 10)
    ...
    def test_required_cargo_id(self):
        ...
        self.assertEqual(rule.strength, RuleStrength.HARD)
        self.assertEqual(rule.params["cargo_id"], "240646")
